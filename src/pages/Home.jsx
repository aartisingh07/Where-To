...
const UserHome = ({ user }) => {
  ...
  return (
    <div className="min-h-screen bg-dark-900 bg-grid pt-24 pb-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-[1600px] w-full mx-auto relative z-10 space-y-8">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <p className="text-white/50 text-sm mt-1">
                  Discover local spots solo, or jump into live squad rooms & DMs.
                </p>
              </div>
            </div>
        ...
        {/* ─── FULL WIDTH 2-COLUMN GRID (Pillar 1 Left | Pillar 2 Right) ─── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          
          {/* COLUMN 1: PILLAR 1 — 📍 INSTANT PLACE EXPLORER */}
          <div className="glass-card p-6 sm:p-8 rounded-3xl border border-cyan-500/30 shadow-glow-teal/10 flex flex-col justify-between space-y-6">
            <div className="space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-white/10">
                <div>
                  <span className="text-xs font-bold uppercase tracking-wider text-cyan-400">Pillar 1 Feature</span>
                  <h2 className="font-display font-bold text-2xl text-white flex items-center gap-2 mt-0.5">
                    <span>📍 Place Exploration & Favorites</span>
                  </h2>
                </div>
                <Link to="/explore" className="btn-secondary text-xs !px-3 !py-1.5 font-bold flex items-center gap-1">
                  Open Explorer <FiArrowRight size={14} />
                </Link>
              </div>

              {/* Pillar 1 Action Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* Explore Spots */}
                <Link to="/explore" className="p-5 rounded-2xl border border-cyan-500/20 bg-cyan-500/5 hover:border-cyan-400/60 hover:bg-cyan-500/10 transition-all duration-300 group flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-white/60 text-xs leading-relaxed mb-3">
                      Instant GPS location venue search with mood filters & distance radius.
                    </p>
                  </div>
                ...
                <Link to="/saved-places" className="p-5 rounded-2xl border border-rose-500/20 bg-rose-500/5 hover:border-rose-400/60 hover:bg-rose-500/10 transition-all duration-300 group flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-10 h-10 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
                        ❤️
                      </div>
                    ...
                    <h3 className="font-display font-bold text-base text-white mb-1 group-hover:text-rose-300 transition-colors">
                      Saved Favorite Places
                    </h3>
                    <p className="text-white/60 text-xs leading-relaxed mb-3">
                      Bookmarked spots with coordinate tags & 1-click Google Maps routing.
                    </p>
                  </div>
              ...
              {/* Outing Recommendation Collections */}
              <div className="pt-2">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold uppercase tracking-wider text-cyan-400">Curated Outing Collections</span>
                  <Link to="/explore" className="text-xs text-cyan-400 hover:underline font-bold">Browse All →</Link>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      subtitle: 'Beaches, Cafes, Sunsets',
                    ...
                    },
                    {
                      ...
                      subtitle: 'Theme Parks, Zoos, Lawns',
                    ...
                    <Link
                      key={cat.id}
                      to={cat.to}
                      className={`p-3.5 rounded-xl border bg-gradient-to-br ${cat.color} transition-all duration-300 hover:-translate-y-0.5 group flex flex-col justify-between cursor-pointer`}
                    >
                      <div>
                        <div className="text-2xl mb-1.5 group-hover:scale-110 transition-transform">{cat.emoji}</div>
                        <h4 className="font-display font-bold text-white text-sm mb-0.5">{cat.title}</h4>
                        <p className="text-white/50 text-[10px] leading-relaxed">{cat.subtitle}</p>
                      </div>
                      <div className="mt-2 text-[11px] font-bold text-cyan-400 group-hover:translate-x-1 transition-transform flex items-center justify-between">
                        <span>Explore</span>
          ...
          {/* COLUMN 2: PILLAR 2 — 👥 SQUAD HANGOUT ROOMS & DMs */}
          <div className="glass-card p-6 sm:p-8 rounded-3xl border border-primary-500/30 shadow-glow-purple/10 flex flex-col justify-between space-y-6">
            <div className="space-y-6">
              <div className="flex items-center justify-between pb-4 border-b border-white/10">
                <div>
                  <span className="text-xs font-bold uppercase tracking-wider text-primary-400">Pillar 2 Feature</span>
                  <h2 className="font-display font-bold text-2xl text-white flex items-center gap-2 mt-0.5">
                    <span>👥 Squad Rooms, Midpoint & Direct DMs</span>
                  </h2>
                </div>
                <div className="flex gap-2">
                  <Link to="/create-room" className="btn-primary text-xs !px-3 !py-1.5 font-bold flex items-center gap-1">
                    Create Room <FiArrowRight size={14} />
                  </Link>
                </div>
              </div>

              {/* Pillar 2 Action Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {/* Create Room */}
                <Link to="/create-room" className="p-5 rounded-2xl border border-primary-500/20 bg-primary-500/5 hover:border-primary-400/60 hover:bg-primary-500/10 transition-all duration-300 group flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-10 h-10 rounded-xl bg-primary-500/10 border border-primary-500/20 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
                        🏠
                      </div>
                    ...
                    <h3 className="font-display font-bold text-base text-white mb-1 group-hover:text-primary-300 transition-colors">
                      Create Hangout Room
                    </h3>
                    <p className="text-white/60 text-xs leading-relaxed mb-3">
                      Start room, share 6-digit code, compute squad midpoint, & vote on games/movies.
                    </p>
                  </div>
                  <div className="flex items-center gap-1 text-xs font-bold text-primary-400 group-hover:translate-x-1 transition-transform">
                ...
                {/* Join Room */}
                <Link to="/join-room" className="p-5 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 hover:border-indigo-400/60 hover:bg-indigo-500/10 transition-all duration-300 group flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
                        🔑
                      </div>
                    ...
                    <h3 className="font-display font-bold text-base text-white mb-1 group-hover:text-indigo-300 transition-colors">
                      Join with Room Code
                    </h3>
                    <p className="text-white/60 text-xs leading-relaxed mb-3">
                      Enter 6-digit code or browse active public lobbies to join squad session.
                    </p>
                  </div>
                ...
                {/* Direct DMs */}
                <Link to="/messages" className="p-5 rounded-2xl border border-teal-500/20 bg-teal-500/5 hover:border-teal-400/60 hover:bg-teal-500/10 transition-all duration-300 group flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div className="w-10 h-10 rounded-xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
                        💬
                      </div>
                      <span className="px-2 py-0.5 rounded-full bg-teal-500/20 text-teal-300 text-[10px] font-bold uppercase tracking-wider">
                        Private Chat
                      </span>
                    </div>
                    <h3 className="font-display font-bold text-base text-white mb-1 group-hover:text-teal-300 transition-colors">
                      Direct Chats & DMs
                    </h3>
                    <p className="text-white/60 text-xs leading-relaxed mb-3">
                      Search friends by username handle, send chat requests, & message 1-on-1.
                    </p>
                  </div>
                  <div className="flex items-center gap-1 text-xs font-bold text-teal-400 group-hover:translate-x-1 transition-transform">
                    <span>Open DMs</span>
                    <FiArrowRight size={12} />
                  </div>
                </Link>
        ...
        {/* ─── BELOW THEM: ACTIVE LOBBIES & UPCOMING OUTING PLANS (Full Width Grid) ─── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start pt-4 border-t border-white/5">
          
          {/* Your Active & Recent Lobbies */}
          <div className="glass-card p-6 sm:p-8 rounded-3xl border border-white/10 animate-slide-up h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
                <h3 className="font-display font-bold text-xl text-white flex items-center gap-2">
                  <span>🏠</span> Your Active & Recent Lobbies
                </h3>
                <span className="text-xs text-white/40 font-medium">1-Click Rejoin</span>
              </div>
          ...
          <div className="glass-card p-6 sm:p-8 rounded-3xl border border-white/10 animate-slide-up h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
                ...
                <span className="text-xs text-white/40 font-medium">Scheduled Plans</span>
              </div>
